...
export const CompressionSettings = ({
  ...
  return <div className="space-y-4 p-5 bg-card rounded-xl border border-border shadow-card">
      ...
      <fieldset disabled={disabled} className="space-y-3">
        ...
        <RadioGroup value={preset} onValueChange={value => onPresetChange(value as CompressionPreset)} className="grid gap-2">
          {presetOrder.map(presetKey => {
          const config = COMPRESSION_PRESETS[presetKey];
          return <div key={presetKey} className="flex items-center space-x-2">
                <RadioGroupItem value={presetKey} id={`compression-${presetKey}`} />
                <Label htmlFor={`compression-${presetKey}`} className="cursor-pointer flex items-center gap-2 flex-1">
      ...
      {currentConfig.warning && <div role="alert" className="flex items-start gap-2 p-3 rounded-lg bg-warning/10 border text-sm border-warning text-secondary-foreground">
          <AlertTriangle className="w-4 h-4 text-warning shrink-0 mt-0.5" aria-hidden="true" />
          <span className="text-secondary-foreground">{currentConfig.warning}</span>
        </div>}
    </div>;
};